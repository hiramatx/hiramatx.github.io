---
layout: page
title: About
permalink: /about/
---

IT support professional with experience in helpdesk and systems administration. I enjoy solving real-world problems, building out home lab environments, and writing up the things I learn along the way.

## Skills

- **OS:** Windows 10/11, Windows Server, Linux (Ubuntu, Arch)
- **Networking:** TCP/IP, DNS, DHCP, VLANs, VPN
- **Tools:** Active Directory, Microsoft 365, ticketing systems
- **Home Lab:** Proxmox, self-hosted services, network segmentation

## Certifications

- CompTIA A+

## Contact

Feel free to reach out via [GitHub](https://github.com/hiramatx) or [LinkedIn](https://www.linkedin.com/in/hiram-martinez/).
